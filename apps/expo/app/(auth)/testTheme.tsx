...
export default function TestTheme() {
  ...
  const { data, error, loading, networkStatus } = useQuery(HEALTH_CHECK);
    ...
    <>
      <SafeAreaView className="flex-1 bg-background dark:bg-backgroundDark">
        <View className="mx-5 mb-5">
          ...
          <Text className="mt-2 text-xl underline dark:text-gray-450 text-slate-400">Color Pallette:</Text>
        </View>
        ...
        <View className="flex-row flex-wrap items-center justify-center flex-1">
          ...
          <Text className="underline text-slate-900 dark:text-white" onPress={() => router.push('/login')}>
            Back to Login
          </Text>
        </View>

        <View className="flex justify-center flex-1 align-middle">
          <Text className="ml-6 text-slate-900 dark:text-white">{JSON.stringify(data, null, 2)}</Text>
        </View>
      </SafeAreaView>
    </>
  );
